...
{
	m_v3Above.y = glm::abs(m_v3Above.y);
...
{
	//Generates forward vector and moves in that direction
	vector3 forwardVector = glm::normalize((m_v3Target - m_v3Position));
...
{
	//Moves vertically relative to the world
...
{
	//Generates right vector from forward and moves in that direction
	vector3 forwardVector = glm::normalize((m_v3Target - m_v3Position));
	vector3 rightVector = glm::cross(glm::normalize(vector3(0.0, 1.0, 0.0)), forwardVector);
...
void Simplex::MyCamera::RotateAroundXAxis(float a_fAngle)
{
	//Rotates the target closer to the up/down vectors. 
	//Doesn't rotate normally as that causes camera flipping.
	vector3 upVector = vector3(0.0f, 0.5f, 0.0f);
	vector3 downVector = vector3(0.0f, 0.5f, 0.0f);
	vector3 vec3FromPosToTarget = m_v3Target - m_v3Position;
	vector3 newRelativeTarget;
	if (a_fAngle > 0)
	{
		newRelativeTarget = glm::normalize(vec3FromPosToTarget + (upVector * a_fAngle)) * glm::length(vec3FromPosToTarget);
	}
	else
	{
		newRelativeTarget = glm::normalize(vec3FromPosToTarget + (downVector * a_fAngle)) * glm::length(vec3FromPosToTarget);
	}
	m_v3Target = m_v3Position + newRelativeTarget;
}

void Simplex::MyCamera::RotateAroundYAxis(float a_fAngle)
{
	//Rotates target relative to position
	vector3 vec3FromPosToTarget = m_v3Target - m_v3Position;
	vector3 newRelativeTarget = glm::rotateY(vec3FromPosToTarget, a_fAngle);
	m_v3Target = m_v3Position + newRelativeTarget;
}
